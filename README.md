...
### Big Data
This project uses an Amazon Web Service's m5a.8xlarge EC2 instance. The instance runs and starts a docker pyspark container. The scripts in `src` issue SQL queries into a SparkContext for cleaning.
...
Lastly, the results are saved as .pickles in `data/cleaned`. The clean .pkl files are moved into AWS S3 storage, for backup / download.
